...
Black-Litterman portfolio construction using LLM response files generated by
`run_returns_metadata_only.py`.

Compared with `blacklitterman_weights.py`, this version expects response files
that were produced without `summary_json` / text summaries. The BL mechanics are
unchanged; only the response input directory, response filename pattern, and
result filename are changed so the experiment is kept separate from the
text-enhanced pipeline.

Expected response file pattern:
    responses_returns_metadata_only/{model}_returns_metadata_only_{start}_{end}.json

Default output file pattern:
    results_returns_metadata_only/{model}_black_litterman_weights_returns_metadata_only_tau_{tau}.csv
"""
import argparse
...
def process_period_for_model(
    ...
    if str(model_name).strip().lower() in {"none", "null"}:
        ...
        # Metadata-only response files generated by run_returns_metadata_only.py.
        resp_path = Path(responses_dir) / f"{model_name}_returns_metadata_only_{start_date}_{end_date}.json"
...
def main():
    ...
    parser.add_argument("--responses_dir", type=str, default="responses_returns_metadata_only")
    parser.add_argument("--dataset_csv", type=str, default=str(ROOT_DIR / "data" / "filtered_sp500_data.csv"))
    parser.add_argument("--results_dir", type=str, default="results_returns_metadata_only")
    parser.add_argument("--risk_free_csv", type=str, default=str(ROOT_DIR / "data" / "DGS1.csv"))
    ...
    for model in args.models:
        ...
        out_path = Path(args.results_dir) / f"{model}_black_litterman_weights_returns_metadata_only_tau_{args.tau}.csv"
